...
FORWARD_CHANNEL_ID = -1002447741574
CHANNEL_ID = -1002447741574
error_channel_id = -1002447741574
    ...
    try:
        ...
        if not user_data or user_data['plan'] == 0:
            bot.send_message(chat_id, "*🚫 Access Denied! dm for free approve @SIDIKI_MUSTAFA_47*\n"  # Access Denied message
                                       ...
                                       "*Contact the owner for assistance: @SIDIKI_MUSTAFA_47.*", parse_mode='Markdown')  # Contact owner message
            return
...
def myinfo_command(message):
    ...
    if not user_data:
        ...
        response += "*For assistance, please contact the owner: @SIDIKI_MUSTAFA_47* "  # Contact owner message
    elif user_data.get('plan', 0) == 0:
        # User found but not approved
        response = "*🔒 Your account is still pending approval!* \n"  # Not approved message
        response += "*Please reach out to the owner for assistance: @SIDIKI_MUSTAFA_47* 🙏"  # Contact owner message
    else:
